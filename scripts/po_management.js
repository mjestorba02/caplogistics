document.addEventListener('DOMContentLoaded', () => {
                ...
                d.pos.forEach(p => {
                    ...
                    row.innerHTML = `
                        ...
                        <td class="px-6 py-3 flex gap-1">
                            ...
                            <button onclick="archivePO(${p.id})" class="bg-orange-600 text-white px-2 py-1 rounded text-xs hover:bg-orange-700" title="Archive">
    ...
    window.archivePO = (id) => {
        if (confirm('Archive this PO?')) {
            fetch('../api/archive_management.php', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ archive_type: 'po', item_id: id, original_table: 'po_management', reason: 'Archived from PO management' }) })
                .then(r => r.json())
                .then(() => fetchPOs());
        }
    };
