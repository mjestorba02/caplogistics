...
<div class="p-6">
    ...
    <div class="flex flex-col md:flex-row justify-between items-center mb-6 gap-3">
        <h1 class="text-3xl font-bold text-gray-800 mb-2 md:mb-0">Maintenance Scheduling</h1>
        <div class="flex flex-wrap items-center gap-3">
            <button id="addBtn" class="bg-indigo-600 text-white px-4 py-2 rounded hover:bg-indigo-700 transition">Schedule Maintenance</button>
    ...
    <!-- Maintenance Grid -->
    <div id="maintGrid" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6"></div>
...
<div id="modal" class="fixed inset-0 hidden justify-center items-center z-50">
  ...
  <div class="bg-white rounded-lg shadow-lg w-full max-w-2xl p-6 relative">
    <div class="flex items-center justify-between mb-4">
      <h2 id="modalTitle" class="text-2xl font-bold">Schedule Maintenance</h2>
      <button class="text-gray-500 hover:text-gray-700 text-2xl leading-none" data-close="button">&times;</button>
    </div>
    <form id="maintForm" class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
        ...
        <div>
            <label class="block text-gray-700 text-sm mb-1">Maintenance Date<span class="text-red-600">*</span></label>
            <input id="f_maintenance_date" type="date" class="w-full border rounded px-3 py-2" required />
        </div>
        <div>
            <label class="block text-gray-700 text-sm mb-1">Type<span class="text-red-600">*</span></label>
            <select id="f_type" class="w-full border rounded px-3 py-2">
...
(function(){
    ...
    const f_maintenance_date = document.getElementById('f_maintenance_date');
    ...
    function openModal(mode, item = null){
        isEditing = mode === 'edit';
        editId = item?.id ?? null;
        modalTitle.textContent = isEditing ? 'Edit Maintenance' : 'Schedule Maintenance';

        f_asset_name.value = item?.asset_name || '';
        f_asset_id.value = item?.asset_id || '';
        f_maintenance_date.value = item?.maintenance_date || '';
        f_type.value = item?.type && ['Preventive','Corrective','Predictive'].includes(item.type) ? item.type : 'Preventive';
        f_status.value = item?.status && ['Scheduled','Completed','Cancelled'].includes(item.status) ? item.status : 'Scheduled';
    ...
    function statusBadge(s){
        ...
        if(s === 'Scheduled') cls = 'bg-blue-200 text-blue-800';
    ...
    function render(){
        if(!items.length){
            grid.innerHTML = `<div class=\"text-gray-500\">No maintenance scheduled</div>`;
            return;
        }
        grid.innerHTML = items.map(m => `
            <div class="bg-white p-6 rounded-lg shadow hover:shadow-lg transition cursor-pointer">
                <h2 class="text-xl font-semibold mb-2">${escapeHtml(m.asset_name)}</h2>
                <p class="text-gray-600 mb-1">Asset ID: ${escapeHtml(m.asset_id)}</p>
                <p class="text-gray-600 mb-1">Date: ${escapeHtml(m.maintenance_date)}</p>
                <p class="text-gray-600 mb-1">Type: ${escapeHtml(m.type)}</p>
                <p class="text-gray-600 mb-2">Status: ${statusBadge(escapeHtml(m.status || 'Scheduled'))}</p>
    ...
    document.addEventListener('click', async (e) => {
        ...
        if(action === 'edit'){
            ...
            if(!confirm('Archive this maintenance?')) return;
            try{
    ...
    maintForm.addEventListener('submit', async (e) => {
        ...
        const payload = {
            ...
            maintenance_date: f_maintenance_date.value,
            type: f_type.value,
            status: f_status.value,
            notes: f_notes.value.trim()
        };
        if(!payload.asset_name || !payload.asset_id || !payload.maintenance_date){
    ...
    function exportCSV(){
        ...
        const headers = ['ID','Asset ID','Asset Name','Date','Type','Status','Notes'];
        const rows = items.map(m => [
            m.id,
            m.asset_id,
            m.asset_name,
            excelText(m.maintenance_date),
            m.type,
            m.status,
            m.notes || ''
        ]);
        ...
        const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = 'maintenance.csv'; document.body.appendChild(a); a.click(); document.body.removeChild(a); URL.revokeObjectURL(url);
    }
