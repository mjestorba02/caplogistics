...
// {
//     $sql = "CREATE TABLE IF NOT EXISTS asset_replacements (
//         id INT AUTO_INCREMENT PRIMARY KEY,
//         asset_id VARCHAR(50) NOT NULL,
//         asset_name VARCHAR(255) NOT NULL,
//         current_age_years INT NOT NULL DEFAULT 0,
...
try {
    switch ($method) {
        case 'GET': {
            ...
            $where = [];
            $params = [];
            if ($id) { $where[] = 'r.id = :id'; $params[':id'] = $id; }
            if ($status) { $where[] = 'r.status = :status'; $params[':status'] = $status; }
            if ($archived === '1') { $where[] = 'r.archived_at IS NOT NULL'; }
            elseif ($archived === '0') { $where[] = 'r.archived_at IS NULL'; }
            if ($q) {
                $where[] = '(r.asset_name LIKE :q OR r.asset_id LIKE :q OR r.notes LIKE :q)';
                $params[':q'] = '%' . sanitize_like($q) . '%';
            }

            $sql = 'SELECT r.* FROM asset_replacements r';
            if ($where) { $sql .= ' WHERE ' . implode(' AND ', $where); }
            $sql .= ' ORDER BY r.replacement_date DESC, r.created_at DESC';
        ...
        case 'POST': {
            ...
            $current_age_years = isset($input['current_age_years']) ? (int)$input['current_age_years'] : 0;
            $replacement_date = trim($input['replacement_date'] ?? '');
            $status = trim($input['status'] ?? 'Planned');
            $notes = trim($input['notes'] ?? '');

            if (!$asset_id || !$asset_name || !$replacement_date) {
                json_response(['status' => 'error', 'message' => 'asset_id, asset_name and replacement_date are required.'], 400);
            }

            $stmt = $conn->prepare('INSERT INTO asset_replacements (asset_id, asset_name, current_age_years, replacement_date, status, notes) VALUES (?, ?, ?, ?, ?, ?)');
            $stmt->execute([$asset_id, $asset_name, $current_age_years, $replacement_date, $status, $notes ?: null]);
            $id = (int)$conn->lastInsertId();

            $stmt2 = $conn->prepare('SELECT * FROM asset_replacements WHERE id = ?');
            $stmt2->execute([$id]);
            $row = $stmt2->fetch(PDO::FETCH_ASSOC);

            json_response(['status' => 'success', 'message' => 'Replacement scheduled.', 'data' => $row], 201);
        }
        case 'PUT': {
            ...
            if ($action === 'archive') {
                $stmt = $conn->prepare("UPDATE asset_replacements SET status = 'Archived', archived_at = NOW() WHERE id = ?");
                $stmt->execute([$id]);
                $stmt2 = $conn->prepare('SELECT * FROM asset_replacements WHERE id = ?');
                $stmt2->execute([$id]);
                $row = $stmt2->fetch(PDO::FETCH_ASSOC);
            ...
            if ($action === 'unarchive') {
                ...
                if (!in_array($restore_status, ['Planned','Approved','Completed','Cancelled'], true)) { $restore_status = 'Planned'; }
                $stmt = $conn->prepare('UPDATE asset_replacements SET status = ?, archived_at = NULL WHERE id = ?');
            ...
            if (isset($input['current_age_years'])) { $fields[] = 'current_age_years = :current_age_years'; $params[':current_age_years'] = (int)$input['current_age_years']; }
            if (isset($input['replacement_date'])) { $fields[] = 'replacement_date = :replacement_date'; $params[':replacement_date'] = trim($input['replacement_date']); }
            ...
            $sql = 'UPDATE asset_replacements SET ' . implode(', ', $fields) . ' WHERE id = :id';
            $params[':id'] = $id;
            $stmt = $conn->prepare($sql);
            $stmt->execute($params);
